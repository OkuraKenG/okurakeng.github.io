  ...
  return (packageName, version) => {
    if (cache.has(packageName)) {
      ...
      if (metadata.time[version]) {
        ...
        return Promise.reject(`Version ${version} not found in cached metadata for ${packageName}`);
      }
    }
          ...
          res.on('end', () => {
            try {
              ...
              if (metadata.time[version]) {
                ...
                reject(`Version ${version} not found in metadata for ${packageName}`);
              }
            } catch (err) {
    ...
    exec(`git show ${defaultBranch}:package-lock.json`, (err, stdout, stderr) => {
      if (err) {
        ...
        try {
          ...
          reject(`Failed to parse package-lock.json from ${defaultBranch}: ${parseErr.message}`);
        }
      }
    });
  });
...
const checkPackagePublishDates = async () => {
  ...
  for (const [pkgName, pkgData] of Object.entries(currentPackages)) {
    ...
    if (mainBranchPackages[pkgName] && mainBranchPackages[pkgName].version === pkgData.version) {
      continue;
    }
